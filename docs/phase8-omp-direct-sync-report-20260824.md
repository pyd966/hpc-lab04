...
按同一作业内交错均值计算，evolve 减少约 1.33%，ABE total 减少约 0.80%（单次总时间的原始值分别为 34.1721/33.9886 和 34.2532/33.8877，原始结果已汇总在本报告表格中）。四次运行的 course check 均为 PASS，逐项输出比较均为 bitwise yes。
