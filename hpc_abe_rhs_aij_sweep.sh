...
OUT_DIR="$ROOT_DIR/profile/abe-rhs-aij-$RUN_ID"
...
RUN_ORDER="${ABE_GEOMETRY_RUN_ORDER:-off on on off}"
...
for value in OFF ON; do
    ...
    ./compile.sh \
        ...
        -DAMSS_ENABLE_RHS_CONNECTION_FUSION=OFF -DAMSS_ENABLE_RHS_AIJ_FUSION="$value" \
        -DAMSS_ENABLE_TWOPUNCTURE_OPENMP=ON -DAMSS_OPT='-O3 -g -fno-omit-frame-pointer -fopt-info-vec-optimized' \
        -DAMSS_TWOPUNCTURE_OPT=-O3 -DAMSS_TWOPUNCTURE_ARCH_FLAGS=-march=native \
        > "$OUT_DIR/build-$name.log" 2>&1
done
export OMP_NUM_THREADS=30 AMSS_OMP_STATIC_BLOCK_TARGET=24 AMSS_OMP_MOVING_BLOCK_TARGET=30
export AMSS_OMP_STATIC_THREADS=24 AMSS_OMP_MOVING_THREADS=30 AMSS_BUILD_DIR="$OUT_DIR/build-off"
