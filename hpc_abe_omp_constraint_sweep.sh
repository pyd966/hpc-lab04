...
OUT_DIR="$ROOT_DIR/profile/abe-omp-constraint-$RUN_ID"
...
RUN_ORDER="${ABE_OMP_SWEEP_RUN_ORDER:-serial parallel parallel serial}"
...
for index in "${!names[@]}"; do
    ...
    echo "=== Build $name: AMSS_ENABLE_OMP_CONSTRAINT_PARALLEL=$value ==="
    ./compile.sh \
        -DAMSS_ENABLE_GPU=OFF \
        -DAMSS_ENABLE_OPENMP=ON \
        -DAMSS_ENABLE_OMP_ONLY=ON \
        -DAMSS_ENABLE_OMP_DIAGNOSTICS=ON \
        -DAMSS_ENABLE_OMP_CONSTRAINT_PARALLEL="$value" \
...
export AMSS_BUILD_DIR="$OUT_DIR/build-serial"
...
printf 'sequence\tcandidate\tevolve_seconds\ttotal_seconds\tconstraint_wall_seconds\taverage_cpus\tipc\tbranch_miss_pct\tL1D_miss_pct\tLLC_load_miss_pct\tdTLB_miss_pct\tbitwise_vs_serial\tcourse_check\n' > "$OUT_DIR/results.tsv"

reference=""
sequence=0
for name in $RUN_ORDER; do
    if [[ "$name" != serial && "$name" != parallel ]]; then
    ...
    if [[ -z "$reference" && "$name" == serial ]]; then
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$sequence" "$name" "$evolve" "$total" "$constraint" "$average" \
        "$ipc" "$branch" "$l1d" "$llc" "$dtlb" "$bitwise" "$course" \
        >> "$OUT_DIR/results.tsv"
done

printf 'candidate\truns\tmean_evolve_seconds\tmean_constraint_wall_seconds\tmean_average_cpus\n' > "$OUT_DIR/summary.tsv"
for name in "${names[@]}"; do
    awk -F '\t' -v candidate="$name" '
        NR > 1 && $2 == candidate {
            count++; evolve += $3; constraint += $5; cpus += $6
        }
        END {
            printf "%s\t%d\t%.6f\t%.6f\t%.3f\n",
                candidate, count, evolve / count, constraint / count, cpus / count
        }
