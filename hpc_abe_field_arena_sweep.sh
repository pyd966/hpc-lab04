...
OUT_DIR="$ROOT_DIR/profile/abe-field-arena-$RUN_ID"
...
EVOLVE_TIME="${ABE_FIELD_ARENA_TIME:-4.0}"
RUN_ORDER="${ABE_FIELD_ARENA_RUN_ORDER:-off on on off}"
...
for value in OFF ON; do
  ...
  ./compile.sh \
    ...
    -DAMSS_ENABLE_RHS_CHI_RICCI_FUSION=OFF -DAMSS_ENABLE_HUGEPAGE_HINT=OFF \
    -DAMSS_ENABLE_BLOCK_FIELD_ARENA="$value" -DAMSS_ENABLE_TWOPUNCTURE_OPENMP=ON \
