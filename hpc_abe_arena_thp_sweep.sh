...
OUT_DIR="$ROOT_DIR/profile/abe-arena-thp-$RUN_ID"
...
EVOLVE_TIME="${ABE_ARENA_THP_TIME:-4.0}"
RUN_ORDER="${ABE_ARENA_THP_RUN_ORDER:-off on on off}"
...
{
  echo "=== transparent huge-page policy ==="
  cat /sys/kernel/mm/transparent_hugepage/enabled 2>/dev/null || true
  cat /sys/kernel/mm/transparent_hugepage/defrag 2>/dev/null || true
  grep -E 'AnonHugePages|FileHugePages|ShmemHugePages|HugePages_Total|Hugepagesize' /proc/meminfo || true
} > "$OUT_DIR/transparent-hugepage.txt"
...
for value in OFF ON; do
  ...
  ./compile.sh \
    ...
    -DAMSS_ENABLE_RHS_CHI_RICCI_FUSION=OFF -DAMSS_ENABLE_BLOCK_FIELD_ARENA=ON \
    -DAMSS_ENABLE_HUGEPAGE_HINT="$value" -DAMSS_ENABLE_TWOPUNCTURE_OPENMP=ON \
