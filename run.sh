...
# The CPU and GPU OJs collect the same run.sh but provide different builds.
# Prefer an explicit mode; otherwise infer the track from the configured
# target. Hardware detection is only a clean-build fallback.
requested_execution_mode="${AMSS_EXECUTION_MODE:-auto}"
case "$requested_execution_mode" in
  auto)
    if [[ -x "$AMSS_BUILD_DIR/ABEGPU" ]] &&
       grep -q '^AMSS_ENABLE_GPU:BOOL=ON$' "$AMSS_BUILD_DIR/CMakeCache.txt" 2>/dev/null; then
      AMSS_EXECUTION_MODE=gpu
    elif [[ -x "$AMSS_BUILD_DIR/ABE" ]] &&
         grep -q '^AMSS_ENABLE_GPU:BOOL=OFF$' "$AMSS_BUILD_DIR/CMakeCache.txt" 2>/dev/null; then
      AMSS_EXECUTION_MODE=cpu
    elif [[ -x "$AMSS_BUILD_DIR/ABEGPU" ]]; then
      AMSS_EXECUTION_MODE=gpu
    ...
    ;;
  cpu|gpu) AMSS_EXECUTION_MODE="$requested_execution_mode" ;;
...
if [[ "$AMSS_EXECUTION_MODE" == "cpu" ]]; then
  ...
  # The OJ may export `--map-by slot:pe=16`. With one GPU rank this can ask
  # Open MPI for more bindable processing elements than the container grants,
  # failing before ABEGPU starts. Keep MPI for the GPU path, but remove only
  # this incompatible mapper request.
