...
OUT_DIR="$ROOT_DIR/profile/abe-kodis-simd-$RUN_ID"
...
OMP_THREADS="${ABE_SIMD_OMP_THREADS:-30}"
STATIC_TARGET="${ABE_SIMD_STATIC_TARGET:-24}"
MOVING_TARGET="${ABE_SIMD_MOVING_TARGET:-30}"
EVOLVE_TIME="${ABE_SIMD_TIME:-4.0}"
RUN_ORDER="${ABE_SIMD_RUN_ORDER:-scalar simd simd scalar}"
...
for index in "${!names[@]}"; do
    ...
    echo "=== Build $name: AMSS_ENABLE_KODIS_SIMD=$value ==="
    ./compile.sh \
        -DAMSS_ENABLE_GPU=OFF \
        -DAMSS_ENABLE_OPENMP=ON \
        -DAMSS_ENABLE_OMP_ONLY=ON \
        -DAMSS_ENABLE_KODIS_SIMD="$value" \
    ...
    objdump -d --disassemble=kodis_ "$build_dir/ABE" > "$OUT_DIR/kodis-$name.asm"
done

export AMSS_BUILD_DIR="$OUT_DIR/build-scalar"
...
printf 'sequence\tcandidate\tevolve_seconds\ttotal_seconds\taverage_cpus\tipc\tbranch_miss_pct\tL1D_miss_pct\tLLC_load_miss_pct\tdTLB_miss_pct\tbitwise_vs_scalar\tcourse_check\n' > "$OUT_DIR/results.tsv"

reference=""
sequence=0
for name in $RUN_ORDER; do
    if [[ "$name" != scalar && "$name" != simd ]]; then
    ...
    if [[ -z "$reference" && "$name" == scalar ]]; then
