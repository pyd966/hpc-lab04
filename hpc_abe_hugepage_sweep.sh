...
OUT_DIR="$ROOT_DIR/profile/abe-hugepage-$RUN_ID"
...
EVOLVE_TIME="${ABE_HUGEPAGE_TIME:-4.0}"
RUN_ORDER="${ABE_HUGEPAGE_RUN_ORDER:-off on on off}"
...
for value in OFF ON; do
  ...
  ./compile.sh \
    ...
    -DAMSS_ENABLE_RHS_CHI_RICCI_FUSION=OFF -DAMSS_ENABLE_HUGEPAGE_HINT="$value" \
...
for name in $RUN_ORDER; do
  ...
  cp "$OUT_DIR/build-$([[ "$value" == ON ]] && echo on || echo off)/ABE" "$run_dir/ABE"
