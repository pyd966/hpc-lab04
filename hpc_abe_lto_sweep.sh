...
OUT_DIR="$ROOT_DIR/profile/abe-lto-$RUN_ID"
...
EVOLVE_TIME="${ABE_LTO_TIME:-4.0}"
RUN_ORDER="${ABE_LTO_RUN_ORDER:-off on on off}"
...
for value in OFF ON; do
  ...
  ./compile.sh \
    ...
    -DAMSS_ENABLE_LTO="$value" -DAMSS_ENABLE_BLOCK_FIELD_ARENA=ON \
    -DAMSS_ENABLE_HUGEPAGE_HINT=OFF -DAMSS_ENABLE_FDERIVS_SIMD=ON \
    -DAMSS_ENABLE_OMP_DIRECT_SYNC=ON -DAMSS_ENABLE_OMP_DIRECT_AMR_TRANSFER=OFF \
    -DAMSS_ENABLE_OMP_DIRECT_AMR_SPLIT=OFF -DAMSS_ENABLE_PROLONG3_PAIRWISE=ON \
    -DAMSS_ENABLE_PROLONG3_SIMD=OFF -DAMSS_ENABLE_RHS_METRIC_FUSION=ON \
    -DAMSS_ENABLE_RHS_GAMMA_FUSION=ON -DAMSS_ENABLE_RHS_CONNECTION_FUSION=OFF \
    -DAMSS_ENABLE_RHS_AIJ_FUSION=OFF -DAMSS_ENABLE_RHS_CHI_RICCI_FUSION=OFF \
